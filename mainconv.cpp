...
int main()  {
  try {
    ...
    fclose(f);
    R2Point *d,*d1,*d2; ConvexHull *c,c2; c = new ConvexHull[3];
    d = new R2Point[n/2]; d1 = new R2Point[n1/2]; d2 = new R2Point[n2/2];
    ...
    c2 = ConvexHull::polyintersect(c,3); Functor g(c2); //g(29.,29.) = 1; 
    n = g.c.size(); delete[] c;
    for(int i = 0; i < n; i++) b.drawLine(g.c.iterConv(i),g.c.iterConv(i + 1),2);
    b.hatch(g.c);
    b.a.Save("res5.bmp"); n = 0;
    //printf("\n"); 
    //printf("(%f,%f)\n",a.iterConv(-1).x,a.iterConv(-1).y);c2.Inside(R2Point(1,1))static_cast <bool>(g(1.,1.))
    if(g(1.,1.)) printf("(1,1) inside Convex hull\n"); else printf("(1,1) outside Convex hull\n");
